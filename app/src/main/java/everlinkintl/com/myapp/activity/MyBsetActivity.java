...
public abstract class MyBsetActivity extends AppCompatActivity {
   ...
   public Toolbar toolbar;
    String[] jumpString = {"LoginActivity", "RegisterActivity"};
    private BroadcastReceiver broadcastReceiver1 = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Cons.RECEIVER_ACTION)) {
                ...
                String forment = intent.getStringExtra(Cons.RECEIVER_PUT_FRAGMENT);
                if (!Tools.isEmpty(forment) && forment.equals("1")) {
                    start();
                }
            }
        }
    ...
    public void currentFragment(int current) {
    ...
    private void start() {
        ...
        runnable = new Runnable() {
            ...
            public void run() {
                ...
                List<ActivityManager.RunningTaskInfo> runningTaskInfo = activityManager.getRunningTasks(100);
                activityName = (runningTaskInfo.get(0).topActivity).toString();
                for (int i = 0; i < jumpString.length; i++) {
                    if (Tools.isEmpty(activityName) || activityName.indexOf(jumpString[i]) > -1) {
                        return;
                    }
                }
                if (activityName.indexOf("MainActivity") > -1 && current == 1) {
                } else {
                    List<Activity> list = ((MyApplication) getApplication()).activityList;
                    boolean isMainActivity = false;
                    for (int i = 0; i < list.size(); i++) {
                        if (list.get(i).getClass().toString().indexOf("MainActivity") > -1) {
                            jump();
                            isMainActivity = true;
                        } else {
                            list.get(i).finish();
        ...
        setCurrent = "1";
        handler.postDelayed(runnable, 10 * 60000);
    }
